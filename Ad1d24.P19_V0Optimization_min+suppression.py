...
earliest_V0, min_peak_time, peak_y, suppression, total_tumor_growth = FOV.find_earliest_peak_and_suppression(y0_base, POV.params_Ad5, FOV.ideal_t_param, FOV.t, V_range)

y0 = y0_base.copy()
y0[3] = earliest_V0  # Update the initial viral load
earliest_y = odeint(FOV.sim, y0, FOV.t, args=(POV.params_Ad5,))

# Plot the optimal solution
if earliest_V0 == None:
    print(f"The earliest tumor volume peak occurs after t = {min_peak_time}")
elif suppression == False:
    print(f"The tumor does not remain suppressed after its peak at t = {min_peak_time}")
else:
    ...
    print(f"Optimal V0: {earliest_V0:.4f} gives the earliest peak time at {min_peak_time:.2f} days"
          f" with a volume of {peak_y:.4f} and the tumor remains suppressed for the given time frame.")